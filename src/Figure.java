...
public class Figure extends Object 
{
    ...
    public boolean attach(SquareBoard board, boolean center) 
    {
        ...
        else 
        {
            ...
            {
                if (getSquareYOffset(i, orientation) - newY > 0) 
                {
                    newY = -getSquareYOffset(i, orientation);
                }
            }
        }
    ...
    public boolean isAllVisible() 
    {
        ...
        {
            if (yPos + getSquareYOffset(i, orientation) < 0) 
            {
                return false;
            }
        }
     ...
     *         false otherwise
     */
    private boolean occupiesSquare(int x, int y) 
    {
        for (int i = 0; i < shapeX.length; i++) 
        {
            if (x == xPos + getSquareXOffset(i, orientation) && y == yPos + getSquareYOffset(i, orientation)) 
            {
                return true;
            }
        }
     ...
     * false.
     *
     * @param figX            the new horizontal position
     * @param figY            the new vertical position
     * @param figOrientation  the new orientation (rotation)
     * 
     * @return true if the figure can be moved, or
     *         false otherwise
     */
    private boolean canMoveTo(int figX, int figY, int figOrientation) 
    {
    	
    	// A figure occupies 4 squares. For the figure to move, it has to be
    	// able to occupy all 4 squares in its new position and orientation.
        for (int sqNum = 0; sqNum < 4; sqNum++) 
        {
            int sqX = figX + getSquareXOffset(sqNum, figOrientation);
            int sqY = figY + getSquareYOffset(sqNum, figOrientation);
            if (!board.isSquareEmpty(sqX, sqY) && !occupiesSquare(sqX, sqY)) 
            {
                return false;
            }
        }
        // All 4 squares can be occupied, so the figure can move.
        return true;
    }
     ...
     */
    private int getSquareXOffset(int square, int orientation) 
    {
     ...
     */
    private int getSquareYOffset(int square, int orientation) 
    {
        switch (orientation % 4) 
        {
	        case 0 :
    ...
    private void paint(Color color) 
    {
        ...
        for (int i = 0; i < shapeX.length; i++) 
        {
            x = xPos + getSquareXOffset(i, orientation);
            y = yPos + getSquareYOffset(i, orientation);
            board.setSquareColor(x, y, color);
        }
    }
    
    /*
     * Added for CSE 15L to facilitate JUnit Tests.
     * return the x-coordinate of the figure on the board.
     */
    public int getXPos()
    {
    	return xPos;
